<template>
  <!-- main container -->
  <div class="max-w-[80rem] h-screen m-auto page relative bg-(--gray-100) flex items-center">
    <div class="w-full">
      <!-- HEADER -->
      ...
      <!-- MAIN CARD -->
      <div
        class="bg-white rounded-2xl sd overflow-hidden border border-(--gray-500)"
      >
        <div class="grid grid-cols-1 lg:grid-cols-2">
          <!-- LEFT COLUMN: MAP -->
          <div
            class="h-[400px] lg:h-full lg:min-h-[600px] relative bg-gray-100"
          >
            ...
            <div
              v-if="order && order.distance"
              class="absolute top-4 left-4 bg-white/90 backdrop-blur px-3 py-1 rounded-lg shadow-md border border-gray-200 z-10"
            >
          ...
          <div class="p-8 flex flex-col justify-between">
            <div class="space-y-6">
              ...
              >
                ...
                <div class="mt-[1rem] flex gap-2">
                  <button
                    class="text-xs bg-white border px-3 py-1 rounded font-bold"
                  >
                    📞 {{ order.pickupPhone }}
                  </button>
                </div>
            ...
            <div class="mt-[2rem]">
              <button
                ...
                :class="[
                  'w-full py-4 text-white font-bold text-lg rounded-xl',
                  currentStep === 3
                    ? 'bg-(--gray-800)'
...
const calculateAndDisplayRoute = () => {
  ...
  const origin =
    order.value.pickup_lat && order.value.pickup_lng
      ? {
          lat: Number(order.value.pickup_lat),
          lng: Number(order.value.pickup_lng),
        }
      : order.value.pick_up_address;

  const destination =
    order.value.destination_lat && order.value.destination_lng
      ? {
          lat: Number(order.value.destination_lat),
          lng: Number(order.value.destination_lng),
        }
